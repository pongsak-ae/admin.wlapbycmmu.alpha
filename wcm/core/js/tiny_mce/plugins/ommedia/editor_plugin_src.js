 ...
 *
 * Copyright 2010, ifirstfix Technology Co., Ltd.
 * 
 */

(function() {
	// Load plugin specific language pack
	//tinymce.PluginManager.requireLangPack('ommedia');

    var each = tinymce.each;
    
	tinymce.create('tinymce.plugins.OMMediaPlugin', {
		...
		init : function(ed, url) {        
			...
			ed.addCommand('mceOMMedia', function() {
				ed.windowManager.open({
					file: url + '/browser.htm',
					width: 720 + parseInt(ed.getLang('ommedia.delta_width', 0)),
					height: 450 + parseInt(ed.getLang('ommedia.delta_height', 0)),
					inline: 1,
					resizable: false,
					scrollbars: false
				}, {
			...
			ed.addButton('ommedia', {title : 'Media', cmd : 'mceOMMedia', image : url + '/img/button.gif'});
		
			ed.onNodeChange.add(function(ed, cm, n) {
				cm.setActive('ommedia', n.nodeName == 'IMG' && isMediaElm(n));
			});
				...
				if (ed && ed.plugins.contextmenu) {
					ed.plugins.contextmenu.onContextMenu.add(function(th, m, e) {
						if (e.nodeName == 'IMG' && /mceItem(Flash|ShockWave|WindowsMedia|QuickTime|RealMedia)/.test(e.className)) {
							m.add({title : 'ommedia.edit', icon : 'ommedia', cmd : 'mceOMMedia'});
						}
					});
				}
			});
	...
	// Register plugin
	tinymce.PluginManager.add('ommedia', tinymce.plugins.OMMediaPlugin);
})();