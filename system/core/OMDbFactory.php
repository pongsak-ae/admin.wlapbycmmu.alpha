...
class OMDatabase
{
    ...
    function __construct($ConnectionString = null) {
		...
		$ConS = explode('::',$ConnectionString);
		$this->_HostName = $ConS[0];
		$this->_DatabaseName = $ConS[1];
		$this->_UserName = $ConS[2];
		$this->_Password = $ConS[3];

		$this->_DB = mysql_connect($this->_HostName, $this->_UserName, $this->_Password, true);

        if ($this->_DB == NULL) {
            $this->_LastErrorMessage = mysql_error();
        } else {
            if ( mysql_select_db($this->_DatabaseName, $this->_DB) == false ){
                $this->_LastErrorMessage = mysql_error($this->_DB);
            }else{
                mysql_query("SET NAMES UTF8 ",$this->_DB);
                mysql_query("SET @@global.sql_mode= 'NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' ",$this->_DB);
            }
        }
    }
