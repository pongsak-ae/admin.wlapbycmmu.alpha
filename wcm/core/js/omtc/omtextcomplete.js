...
$.OmTextComplete = function(element, _options) {
	...
	decode: function(o){ if (o == undefined) return []; return o.split(','); }
  }, _options);
	...
	var enablePlugin = function(name, options){
		self.plugins[name] = new $.OmTextComplete[camelCase(capitalize(name))](self, options);
	};
	...
	var create = function(klass, value, opt){
		...
		return new $.OmTextCompleteBit(klass, value, self, $.extend(true, options.bitsOptions[klass], opt));
	};
	...
	var getBit = function(obj){
		return (obj.type && (obj.type == 'editable' || obj.type == 'box')) ? obj : $(obj).data('omtextcomplete:bit');
	};
...
$.OmTextCompleteBit = function(type, value, omtextcomplete, _options){
	...
	var self = this;
	var init = function(){
		prefix = omtextcomplete.getOptions().prefix + '-bit';
		typeprefix = prefix + '-' + type;
		bit = $('<li />').addClass(prefix).addClass(typeprefix)
			.data('omtextcomplete:bit', self)
			.hover(function(){
	...
	var fireBitEvent = function(type){
		...
		omtextcomplete.fireEvent('bit' + type, self).fireEvent('bit' + name + type, self);
		return self;
	};
  ...
  this.is = function(t){
	...
	if (type == 'editable'){
		...
		var toBox = function(){
			...
			var b = omtextcomplete.create('box', value);
			if (b){
				b.inject(bit, 'before');
...
$.fn.extend({
	...
	omtextcomplete: function(options){
		return this.each(function(){
			new $.OmTextComplete(this, options);
		});
	}
...
$.OmTextComplete.Autocomplete = function(omtextcomplete, _options){
  var index, prefix, method, container, list, values = [], searchValues = [], results = [], placeholder = false, current, currentInput, hidetimer, doAdd, currentSearch, currentRequest;
	...
	var init = function(){
		omtextcomplete.addEvent('bitEditableAdd', setupBit)
		...
		if ($.browser.msie) omtextcomplete.setOptions({bitsOptions: {editable: {addOnBlur: false}}});
		prefix = omtextcomplete.getOptions().prefix + '-autocomplete';
		method = $.OmTextComplete.Autocomplete.Methods[options.method];
		container_width = omtextcomplete.getContainer().width();
		if (container_width == 0) container_width = 286;
		container = $('<div class="'+ prefix +'" />').width(container_width).appendTo(omtextcomplete.getContainer());
	...
	var showResults = function(search){
		...
		if (omtextcomplete.getOptions().unique){
			results = $.grep(results, function(v){ return omtextcomplete.isDuplicate(v) == -1; });
		}
		hidePlaceholder();
	...
	var addResult = function(r, searched){
		var element = $('<li class="'+ prefix +'-result" />').html(r[3] ? r[3] : r[1]).data('omtextcomplete:auto:value', r);
	...
	var addCurrent = function(){
		var value = current.data('omtextcomplete:auto:value');
		var b = omtextcomplete.create('box', value.slice(0, 3));
		if (b){
			b.autoValue = value;
...
$.OmTextComplete.Autocomplete.Methods = {
