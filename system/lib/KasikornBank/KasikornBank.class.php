...
class KasikornBank {
	...
	public function request ($gateway, $path, $data = null) {
		...
		// $this->response = iconv('UTF-8', 'UTF-8//TRANSLIT', curl_exec($handle));
	...
	public function request_login ($gateway, $path, $data = null) {
		$handle = curl_init($gateway.ltrim($path, "/"));
		if (!is_null($data)) {
			if (is_array($data) && !is_null($this->_TOKEN)) {
				$data = array_merge(array("org.apache.struts.taglib.html.TOKEN" => $this->_TOKEN), $data);
	...
	public function Login () {
		...
		$this->request_login($this->online_gateway, "/login.do", array(
			"tokenId" => "0",
		...
		if ($this->http_code === 302) {
			$this->request_login($this->online_gateway, "/ib/redirectToIB.jsp");
			if ($this->http_code === 200) {
				if (preg_match("/<input type=\"hidden\" name=\"txtParam\" value=\"([a-z0-9]{192})\" \/>/", $this->response, $matches)) {
					$this->request_login($this->ebank_gateway, "/security/Welcome.do", array(
						"txtParam" => $matches[1]
					));
